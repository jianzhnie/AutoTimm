...
def unzip(zip_file_path, root='./', strict=False):
    """Unzips files located at `zip_file_path` into parent directory specified
    by `root`."""
...
def untar(tar_file_path, root='./', strict=False):
    """Untars files located at `tar_file_path` into parent directory specified
    by `root`."""
    root = os.path.expanduser(root)
...
class _DisplayablePath:
    ...
    def displayable(self):
        ...
        _filename_prefix = (
            ...
            if self.is_last else self.display_filename_prefix_middle)
...
class PathTree:
    ...
    def __init__(self, root, max_depth=1):
        self._disp_path = _DisplayablePath.make_tree(
            Path(root), max_depth=max_depth)
