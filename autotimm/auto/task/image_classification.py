...
from ..estimators import BaseEstimator, ImageClassificationEstimator
...
@dataclass
class DefaultConfig:
    model: Union[type(None), str,
                 ag.Space] = ag.Categorical('resnet50', 'efficientnet_b0')
...
def _train_image_classification(args, reporter):
    ...
    try:
        logging.info('=' * 100)
        logging.info('Start to training image classification trial %s ',
                     args.task_id)
        ...
        if model:
            ...
            if torch_model_list and model in torch_model_list:
                ...
                if not torch_model_list:
                    raise ValueError(
                        'Model not found in timm model zoo. Install torch and timm if it supports the model.'
                    )

    assert dispatcher in (
        'torch'
    ), 'custom net needs to be of type either torch.nn.Module or mx.gluon.Block'
    ...
    try:
        ...
        if final_fit:
            ...
            if os.path.isdir(log_dir):
                is_valid_dir_fn = lambda d: d.startswith(
                    '.trial_') and os.path.isdir(os.path.join(log_dir, d))
                trial_dirs = [
                    d for d in os.listdir(log_dir) if is_valid_dir_fn(d)
                ]
                best_checkpoint = ''
                best_acc = -1
                result = {}
                ...
                if best_checkpoint:
                    ...
                    print(
                        '=' * 30, 'Set %s model checkpoint from %s' %
                        (estimator, best_checkpoint))
        ...
        if not final_fit:
            ...
            result = estimator.fit(
                ...
                val_data=val_data,
                time_limit=wall_clock_tick - tic)
            with open(os.path.join(trial_log_dir, valid_summary_file),
                      'w') as f:
                json.dump(result, f)
            ...
            if task is not None:
                ...
                json_file_name = task + '_dataset-' + dataset + '_trials-' + str(
                    num_trials) + '_' + time_str + '.json'
                with open(json_file_name, 'w') as json_file:
                    json_file.write(json_str)
                logging.info(
                    'Config and result in this trial have been saved to %s.',
                    json_file_name)
...
class ImageClassification(BaseTask):
        ...
        if not config:
            ...
            if not config.get('dist_ip_addrs', None):
                ...
                config = default_config.merge(
                    config, allow_new_key=True).asdict()
    ...
    def fit(self,
        ...
        config['log_dir'] = os.path.join(
            config.get('log_dir', os.getcwd()),
            str(uuid.uuid4())[:8])
        self.scheduler_options['checkpoint'] = os.path.join(
            config['log_dir'], 'exp1.ag')
