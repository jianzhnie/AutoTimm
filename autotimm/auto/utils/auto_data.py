...
from ...utils.filesystem import PathTree, untar, unzip
...
def url_data(url,
    ...
    if fname.endswith('.zip'):
        folder = unzip(
            fname,
            root=root if root else extract_root,
            strict=overwrite_folder)
...
class Config:
    ...
    def load_config(self):
        """load and return config if version equals 2 in existing, else create
        new config."""
    ...
    def create_config(self, cfg=None):
        ...
        config = {
            'data_path': str(self.config_path / 'datasets'),
    ...
    def save_file(self, config):
        """save config file at default config location
        `~/.autotorch/config.yml`."""
...
class URLs():
    ...
    def path(url='.', c_key='archive'):
        ...
        fname = url.split('/')[-1]
        local_path = URLs.LOCAL_PATH / ('models' if c_key == 'models' else
                                        'datasets') / fname
