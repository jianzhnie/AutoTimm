...
from autotimm.auto.utils import _suggest_load_context
from autotimm.utils.random import setup_seed
...
class BaseEstimator:
    """This is the base estimator for autotimm.auto.Estimators.
    ...
    def __init__(self, config, logger=None, reporter=None, name=None):
        ...
        # logdir
        logdir = config.pop('log_dir', None) if isinstance(config,
                                                           dict) else None
        if logdir:
            ...
            self._logdir = os.path.join(
                os.getcwd(),
                name.lower() + datetime.now().strftime('-%m-%d-%Y'))

        # finalize config
        cfg = self._default_cfg.merge(
            config)  # config can be dict or yaml file
    ...
    def fit(self,
        ...
        if time_limit is None:
            ...
            raise TypeError(
                f'Invalid type `time_limit={time_limit}`, int/float or None expected'
            )
        if not resume:
    ...
    def reset_ctx(self, ctx=None):
                ...
                if hasattr(self.net, 'reset_ctx'):
                    ...
                    if isinstance(self.net, torch.nn.DataParallel):
                        self.net = torch.nn.DataParallel(
                            self.net.module,
                            device_ids=[ctx.index for ctx in ctx_list])
        ...
        if not done:
            raise RuntimeError(
                'Unable to reset_ctx, no `mxnet` and `pytorch`.')
    ...
    def save(self, filename):
        ...
        with open(filename, 'wb') as fid:
            try:
                ...
                self._logger.warning(
                    f'Unable to pickle object due to the reason: {str(e)}. This object is not saved.'
                )
                return
