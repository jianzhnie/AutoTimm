...
from autogluon.core.utils import get_gpu_count, set_logger_verbosity, verbosity2loglevel
...
from autogluon.vision.configs.presets_configs import _check_gpu_memory_presets, unpack
...
class ImagePredictor(object):
    """AutoTimm Predictor for predicting image category based on their whole
    contents.
    ...
    def fit(self,
        ...
        presets : list or str or dict, default = ['medium_quality_faster_train']
            ...
            It is recommended to specify presets and avoid specifying most other `fit()` arguments or model hyperparameters prior to becoming familiar with autotimm.
            ...
            In-depth Preset Info:
                ...
                medium_quality_faster_train={
                    ...
                    This is the default preset in autotimm, but should generally only be used for quick prototyping.
        ...
        **kwargs :
        ...
        if tuning_data is not None and not tuning_data_validated:
            ...
            if isinstance(tuning_data, self.Dataset):
                tuning_data = self.Dataset(
                    tuning_data, classes=tuning_data.classes)
        ...
        if log_level > logging.INFO:
            logging.getLogger(
                'autotimm.auto.tasks.image_classification').propagate = False
        ...
        task = ImageClassification(
            config=config, problem_type=self._problem_type)
    ...
    def _validate_data(self, data):
        """Check whether data is valid, try to convert with best effort if
        not."""
        ...
        if not (hasattr(data, 'classes') and hasattr(data, 'to_pytorch')):
            if isinstance(data, pd.DataFrame):
                ...
                if self._label_inner in data.columns and 'image' in data.columns:
                    ...
                    if self._problem_type in [MULTICLASS, BINARY]:
                        ...
                        infer_classes = []
                        logger.log(20,
                                   'Set classes = [] for regression problems')
            ...
            if self._problem_type in [MULTICLASS, BINARY]:
                ...
                unique_labels = sorted(
                    data[self._label_inner].unique().tolist())
                if not (all(ulabel in orig_classes
    ...
    def _validate_kwargs(self, kwargs):
        ...
        valid_kwargs = {
            'holdout_frac', 'random_state', 'nthreads_per_trial',
            'ngpus_per_trial', 'hyperparameter_tune_kwargs'
        }
        invalid_kwargs = []
        for key in kwargs:
        ...
        if invalid_kwargs:
            raise KeyError(
                f'Invalid kwargs specified: {invalid_kwargs}. Valid kwargs names: {list(valid_kwargs)}'
            )
    ...
    def predict_proba(self, data, batch_size=32, as_pandas=True):
        ...
        y_pred_proba = self._classifier.predict(
            data, batch_size=batch_size, with_proba=True)
        if isinstance(data, pd.DataFrame):
    ...
    def predict(self, data, batch_size=32, as_pandas=True):
        ...
        if self._problem_type in [REGRESSION]:
            return self.predict_proba(
                data, batch_size=batch_size, as_pandas=as_pandas)
    ...
    def predict_feature(self, data, batch_size=32, as_pandas=True):
        """Predict images visual feature representations, return the features
        as numpy (1xD) vector.
    ...
    def evaluate(self, data):
        ...
        if isinstance(data, pd.DataFrame) and not isinstance(
            ...
            if self._problem_type in [MULTICLASS, BINARY]:
                data = ImageClassification.Dataset(
                    data, classes=self._train_classes)
            else:
                data = ImageClassification.Dataset(data, classes=[])
    ...
    @classmethod
    def list_models(cls):
        """Get the list of supported model names in model zoo that can be used
